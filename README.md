...
Today the catalog includes `900` rules across `18` categories, with `recommended`, `strict`, `security`, and `experimental` presets.
...
| Correctness | 23 | Async bugs, null access, control-flow mistakes, missing fallbacks, race conditions |
...
The catalog also includes these `121` additional rules beyond the highlights above.
